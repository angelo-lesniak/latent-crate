...
- Host-mounted storage, saved custom-node dependencies, source/build records,
  and CI configuration checks.
- Network-disabled, containerized custom-node snapshotting without a host Python
  requirement.
- An isolated runtime Python environment compatible with PEP 668, reviewed
  PyTorch/Manager dependency constraints, and a consolidated validation-status
  record.
...
- Shared-library FFmpeg builds with x265, expanded media verification, reusable
  BuildKit/CI caches, and edge-only pinned CUDA TorchCodec support.
...
- Opt-in API-node/browser-offline mode, Manager-sharing and metadata controls,
  remote-bind guards, runtime containment, and configurable CPU CI runners.
...
- Reviewed VCS requirement overrides, including a cached SAM2 commit, plus an
  exact offline package lock for the final custom-node dependency install.
